...
export function SpecialtyTemplate({data, path, feature, children}:{data:Specialty; path?:string; feature?:ReactNode; children?:ReactNode}) {
  ...
  {data.heroImage && <figure className="specialty-body-visual shell"><div><img src={data.heroImage} srcSet={heroSrcSet} sizes="(max-width: 900px) calc(100vw - 30px), 70vw" width={1122} height={1402} loading="lazy" decoding="async" alt={data.heroImageAlt}/></div><figcaption><span>Imagem ilustrativa</span><p>Representação de uma consulta dermatológica. A avaliação e a conduta são individualizadas.</p></figcaption></figure>}
  {feature}
