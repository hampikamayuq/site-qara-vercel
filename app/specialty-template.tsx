...
const introSectionLabels = ["Entenda a condição", "Quando procurar avaliação", "Como conduzimos o cuidado"];
...
export function SpecialtyTemplate({data, path, feature, children}:{data:Specialty; path?:string; feature?:ReactNode; children?:ReactNode}) {
  ...
  const heroBackgroundImage = data.heroImage?.replace(/\.webp$/, "-hero.webp");
  const heroBackgroundSrcSet = heroBackgroundImage
    ? `${heroBackgroundImage.replace(/\.webp$/, "-1024.webp")} 1024w, ${heroBackgroundImage} 1672w`
    : undefined;
  const schema = path ? {
  ...
  const heroCopy = <div className={data.heroImage ? "specialty-photo-hero-copy" : undefined}><p className="kicker">{data.eyebrow}</p><h1>{data.title}</h1><p className="lead">{data.lead}</p><div className="actions"><a className="button craft-primary" href={appointmentHref} data-conversion-event="whatsapp_click" data-conversion-placement="hero" data-conversion-variant="schedule" data-conversion-context="specialty" data-conversion-specialty={specialty}>Consultar horários</a><a className="quiet-link" href="#atendimentos">Ver condições atendidas ↓</a></div></div>;
  ...
  {data.heroImage && heroBackgroundImage ? <section className="specialty-hero specialty-photo-hero" data-conversion-event="specialty_view" data-conversion-placement="hero" data-conversion-context="specialty" data-conversion-specialty={specialty}><picture className="specialty-photo-hero-media"><source media="(max-width: 620px)" srcSet={heroSrcSet}/><source srcSet={heroBackgroundSrcSet}/><img src={heroBackgroundImage} width={1672} height={941} fetchPriority="high" alt="" aria-hidden="true"/></picture><Breadcrumb trail={[[data.eyebrow, ""]]}>{data.eyebrow}</Breadcrumb><div className="shell specialty-photo-hero-grid">{heroCopy}</div></section> : <section className="specialty-hero" data-conversion-event="specialty_view" data-conversion-placement="hero" data-conversion-context="specialty" data-conversion-specialty={specialty}><Breadcrumb trail={[[data.eyebrow, ""]]}>{data.eyebrow}</Breadcrumb><div className="shell specialty-hero-grid">{heroCopy}<div className={`specialty-portrait portrait-${data.doctor.split(" ")[1].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()}`}><img src={heroImage} srcSet={heroSrcSet} sizes={heroSrcSet && "(max-width: 900px) 100vw, 45vw"} width={1122} height={1402} fetchPriority="high" alt={`${data.doctor}, dermatologista da Clínica QARA`}/></div></div></section>}
  {data.heroImage ? <section className="section shell specialty-story"><header className="specialty-story-heading"><h2>{data.introTitle}</h2></header><figure className="specialty-story-visual"><div><img src={data.heroImage} srcSet={heroSrcSet} sizes="(max-width: 620px) calc(100vw - 30px), (max-width: 900px) calc(100vw - 60px), 42vw" width={1122} height={1402} loading="lazy" decoding="async" alt={data.heroImageAlt}/></div><figcaption><span>Imagem ilustrativa</span><p>Representação de uma consulta dermatológica. A avaliação e a conduta são individualizadas.</p></figcaption></figure><div className="specialty-story-copy">{data.intro.map((paragraph,index)=><section key={paragraph}><h3>{introSectionLabels[index] ?? "Informação importante"}</h3><p>{paragraph}</p></section>)}</div></section> : <section className="section shell intro-split"><div><h2>{data.introTitle}</h2></div><div>{data.intro.map(p=><p key={p}>{p}</p>)}</div></section>}
  {feature}
  ...
  {data.team && data.team.length > 0 && <section className="section shell specialty-team"><div><h2>Cuidado em equipe.</h2></div><div className="specialty-team-grid">{data.team.map(([doctor, credential, doctorImage, reason]) => <article key={doctor}><img src={doctorImage} srcSet={portraitSrcSet[doctorImage]} sizes={portraitSrcSet[doctorImage] && "160px"} width={415} height={640} alt={doctor} loading="lazy" decoding="async"/><div><p className="kicker">{reason}</p><h3>{doctor}</h3><p className="credential">{credential}</p><a href={`/equipe/${slugifyDoctor(doctor)}`}>Conhecer formação →</a></div></article>)}</div></section>}
  {children}
